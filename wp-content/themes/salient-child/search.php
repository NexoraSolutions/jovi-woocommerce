		...
		<div class="row">
			<div class="col span_6 ">
				<div class="inner-wrap">
					<h1><?php echo esc_html__( 'Resultados de', 'salient' ); ?> <span>"<?php echo esc_html( get_search_query( false ) ); ?>"</span></h1>
					<?php
					if ( $wp_query->found_posts ) {
						echo '<span class="result-num">' . esc_html( $wp_query->found_posts ) . ' ' . esc_html__( 'resultados encontrados', 'salient' ) . '</span>'; }
						?>
					</div>
				</div>
			</div>
			...
			<div class="col span_<?php echo esc_attr( $search_col_span ); // WPCS: XSS ok. ?>">
				...
				<div id="search-results" data-layout="<?php echo esc_attr( $search_results_layout ); ?>">
					...
					endwhile;
						...
						echo '<h3>' . esc_html__( 'Lo sentimos, no se han encontrado resultados.', 'salient' ) . '</h3>';
						echo '<p>' . esc_html__( 'Prueba con otras palabras.', 'salient' ) . '</p>';
						get_search_form();
