				 ...
				 <div class="inner-wrap">
					 <div class="col span_12">
						  <form role="search" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="GET">
							...
							$placeholder_text = ( 'material' === $theme_skin ) ? esc_attr__( 'Buscar', 'salient' ) : esc_attr__( 'Escribe algo...', 'salient' );
							...
							?>
							 <input type="text" name="s" <?php if ( 'yes' === $ajax_search ) { echo 'id="s"'; } ?> value="" aria-label="<?php echo esc_attr__( 'Buscar', 'salient' ); ?>" placeholder="<?php echo esc_attr($placeholder_text); ?>" />
							 <?php
							} else {
								?>
								<input type="text" name="s" <?php if ( 'yes' === $ajax_search ) { echo 'id="s"'; } ?> value="<?php echo esc_attr($placeholder_text); ?>" aria-label="<?php echo esc_attr__( 'Buscar', 'salient' ); ?>" data-placeholder="<?php echo esc_attr($placeholder_text); ?>" />
							<?php } ?>

						<?php
						if ( 'ascend' === $theme_skin && 'no' === $ajax_search && false !== $bottom_helper_text ) {
							echo '<span><i>' . __( 'Pulsa INTRO para empezar a buscar', 'salient' ) . '</i></span>'; }
						if ( 'material' === $theme_skin && false !== $bottom_helper_text ) {
							echo '<span>' . esc_html__( 'Pulsa INTRO para buscar o ESC para cerrar', 'salient' ) . '</span>'; }
						?>
